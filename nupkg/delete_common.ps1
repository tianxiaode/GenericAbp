# Paths

# List of projects
$packages = (

    # modules/w2ui
    "Generic.Abp.W2Ui",
    
    # modules/jspreadsheet
    "Generic.Abp.Jspreadsheet",

    # modules/businessexception
    "Generic.Abp.BusinessException",

    # modules/helper
    "Generic.Abp.Helper.Common",
    "Generic.Abp.Helper.File",

    # modules/extmenu
    #"Generic.Abp.ExtMenu.Application.Contracts",
    #"Generic.Abp.ExtMenu.Application",
    #"Generic.Abp.ExtMenu.HttpApi",

    # modules/extresource
    "Generic.Abp.ExtResource.Application.Contracts",
    "Generic.Abp.ExtResource.Application",
    "Generic.Abp.ExtResource.HttpApi",

    # modules/texttemplate
    #"Generic.Abp.TextTemplate",

    # modules/enumeration
    "Generic.Abp.Enumeration.Domain.Shared",
    "Generic.Abp.Enumeration.Application.Contracts",
    "Generic.Abp.Enumeration.Application",
    "Generic.Abp.Enumeration.HttpApi",

    # modules/ddd
    "Generic.Abp.DDD.Domain.Shared",
    "Generic.Abp.DDD.Domain",

    # modules/application
    "Generic.Abp.Application",
    
    # modules/identity
    "Generic.Abp.Identity.Domain.Shared",
    "Generic.Abp.Identity.Domain",
    "Generic.Abp.Identity.EntityFrameworkCore",
    "Generic.Abp.Identity.Application.Contracts",
    "Generic.Abp.Identity.Application",
    "Generic.Abp.Identity.HttpApi",

    # modules/identityServer
    "Generic.Abp.IdentityServer.Domain.Shared",
    "Generic.Abp.IdentityServer.Application.Contracts",
    "Generic.Abp.IdentityServer.Application",
    "Generic.Abp.IdentityServer.HttpApi",
    "Generic.Abp.IdentityServer.Web",

    # modules/FileManagement
    "Generic.Abp.FileManagement.Domain.Shared",
    "Generic.Abp.FileManagement.Domain",
    "Generic.Abp.FileManagement.EntityFrameworkCore",
    "Generic.Abp.FileManagement.Application.Contracts",
    "Generic.Abp.FileManagement.Application"


    # modules/themes
    #"Generic.Abp.Themes.Shared",
    #"Generic.Abp.Themes",

    # modules/account
    #"Generic.Abp.Account.Application.Contracts",
    #"Generic.Abp.Account.Application",
    #"Generic.Abp.Account.HttpApi",
    #"Generic.Abp.Account.Identity.Web",
    #"Generic.Abp.Account.Web",
    #"Generic.Abp.Account.IdentityServer.Web"
        
)

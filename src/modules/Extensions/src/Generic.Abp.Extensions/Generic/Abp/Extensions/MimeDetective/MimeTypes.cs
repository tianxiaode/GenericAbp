...
namespace Generic.Abp.Extensions.MimeDetective
{
    public static class MimeTypes
    {
        ...
        public static readonly FileType WordX = new([0x50, 0x4B, 0x03, 0x04], "docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", 512);
        public static readonly FileType Excel = new([0x09, 0x08, 0x10, 0x00, 0x00, 0x06, 0x05, 0x00], "xls", "application/excel", 512);
        public static readonly FileType ExcelX = new([0x50, 0x4B, 0x03, 0x04], "xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", 512);
        public static readonly FileType Ppt = new([0xFD, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00], "ppt", "application/mspowerpoint", 512);
        public static readonly FileType PptX = new([0x50, 0x4B, 0x03, 0x04], "pptx", "application/vnd.openxmlformats-officedocument.presentationml.presentation", 512);
        public static readonly FileType Pdf = new([0x25, 0x50, 0x44, 0x46], "pdf", "application/pdf");
        public static readonly FileType Xml = new([0x72, 0x73, 0x69, 0x6F, 0x6E, 0x3D, 0x22, 0x31, 0x30, 0x22, 0x3F, 0x3E], "xml", "text/xml");
        #endregion
        ...
        public static readonly FileType Mp4 = new([0x00, 0x00, 0x00, 0x20, 0x66, 0x74, 0x79, 0x70, 0x69, 0x73, 0x6F, 0x6D], "mp4", "video/mp4");
        ...
        public static readonly List<FileType> Types = [Word, WordX, Excel, ExcelX, Ppt, PptX, Jpg, Png, Pdf, Gif, Bmp, Mp4, Mkv, Mp3, Wav];
        ...
        public static async Task<FileType?> GetFileTypeAsync(this byte[] header, List<FileType> allowedFileTypes)
        {
            ...
            {
                if (fileType.Header != null && await GetFileMatchingCountAsync(header, fileType.Header, fileType.HeaderOffset) == fileType.Header.Length)
                {
                    return fileType;
                }
            }
        ...
        private static Task<int> GetFileMatchingCountAsync(byte[] fileHeader, byte[] header, int offset)
        {
            for (var i = 0; i < header.Length; i++)
            {
                ...
                {
                    return Task.FromResult(0); // 直接返回0，避免不必要的循环
                }
            }
            return Task.FromResult( header.Length); // 直接返回匹配的长度
        }

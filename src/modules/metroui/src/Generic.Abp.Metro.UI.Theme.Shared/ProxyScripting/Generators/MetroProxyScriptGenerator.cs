...
{
    public class MetroProxyScriptGenerator : IProxyScriptGenerator, ITransientDependency
    {
        ...
        /// "jquery".
        /// </summary>
        public const string Name = "metro";

        private readonly DynamicJavaScriptProxyOptions _dynamicJavaScriptProxyOptions;

        public MetroProxyScriptGenerator(IOptions<DynamicJavaScriptProxyOptions> dynamicJavaScriptProxyOptions)
        {
            ...
            ActionApiDescriptionModel action, string normalizedActionName)
        {
            ...
            script.AppendLine(
                $"    {controllerName}{ProxyScriptingJsFuncHelper.WrapWithBracketsOrWithDotPrefix(normalizedActionName.RemovePostFix("Async").ToCamelCase())} = function({parameterList}) {{");
            ...
            if (versionParam != null)
            {
                ...
                script.AppendLine(
                    $"      var {ProxyScriptingJsFuncHelper.NormalizeJsVariableName(versionParam.Name)} = api_version ? api_version : '{version}';");
            }

            script.AppendLine("      return abp.ajax(_.defaultsDeep( {");
            ...
            script.AppendLine(ajaxParamsIsFromForm
                ? "      }, _.defaultsDeep( {}, " + dataType +
                  "{ contentType: 'application/x-www-form-urlencoded; charset=UTF-8' }, ajaxParams)));"
                : "      }, " + dataType + "ajaxParams));");
        ...
        private static void AddAjaxCallParameters(StringBuilder script, ActionApiDescriptionModel action)
        {
            ...
            script.AppendLine("        url: abp.appPath + '" + ProxyScriptingHelper.GenerateUrlWithParameters(action) +
                              "',");
            script.Append("        color: '" + httpMethod + "'");
        ...
        private static Dictionary<ActionApiDescriptionModel, string> CalculateNormalizedActionNames(
            Dictionary<string, ActionApiDescriptionModel> actions)
        {

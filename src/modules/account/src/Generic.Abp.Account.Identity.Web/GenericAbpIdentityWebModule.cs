...
namespace Generic.Abp.Account.Identity.Web
{
    ...
    public class GenericAbpIdentityWebModule : AbpModule
    {
        public override void PreConfigureServices(ServiceConfigurationContext context)
        {
            ...
            {
                options.AddAssemblyResource(typeof(IdentityResource), typeof(GenericAbpIdentityWebModule).Assembly);
            });

            PreConfigure<IMvcBuilder>(mvcBuilder =>
            {
                mvcBuilder.AddApplicationPartIfNotExists(typeof(GenericAbpIdentityWebModule).Assembly);
            });
        }

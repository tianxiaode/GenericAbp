<template>
    <AccountForm ref="formRef" :rules="rules" title="Pages.ForgotPassword.ForgotPassword" v-model="formData">
        ...
        <el-form-item prop="email" clearable>
            <el-input v-model="formData.email" :placeholder="t('Pages.ForgotPassword.Email')" @keyup.enter="handleSubmit">
                <template #prefix>
                    <i class="fa fa-at"></i>
                </template>
            </el-input>
        </el-form-item>
        <el-button ref="submitBtn" type="primary" @click="handleSubmit" class="w-full"
            :disabled="disabled"
        >
